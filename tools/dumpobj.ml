...
let rec print_obj x =
  if Obj.is_block x then begin
    ...
    if tag = Pobj.string_tag then
        printf "%S" (Obj.magic x : string)
    else if tag = Pobj.double_tag then
        printf "%.12g" (Obj.magic x : float)
    else if tag = Pobj.double_array_tag then begin
        ...
        printf "|]"
    end else if tag = Pobj.custom_tag && same_custom x 0l then
        printf "%ldl" (Obj.magic x : int32)
    else if tag = Pobj.custom_tag && same_custom x 0n then
        printf "%ndn" (Obj.magic x : nativeint)
    else if tag = Pobj.custom_tag && same_custom x 0L then
        printf "%LdL" (Obj.magic x : int64)
    else if tag < Pobj.no_scan_tag then begin
        printf "<%d>" (Obj.tag x);
